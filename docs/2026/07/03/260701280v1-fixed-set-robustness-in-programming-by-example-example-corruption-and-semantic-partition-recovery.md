...
## TLDR
This paper formalizes fixed-set worst-case corruption in programming-by-example, showing that low-margin tasks are vulnerable to targeted attacks that stochastic noise models miss, and proposes a defense (version-space partition aggregation) that recovers only when the clean semantics retain a partition vote margin.
