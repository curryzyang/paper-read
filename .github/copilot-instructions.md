...
Zotero-arXiv-Daily recommends new arXiv/bioRxiv/medRxiv papers based on a user's Zotero library. It computes embedding similarity between new papers and the user's existing library, generates TLDRs via LLM, and publishes results as a static GitHub Pages report. Designed to run as a GitHub Actions workflow at zero cost.
...
6. **Publish web report** → static Markdown/JSON under `docs/` and `archive/`
