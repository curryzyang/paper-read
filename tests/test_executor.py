...
def test_run_end_to_end(config, monkeypatch, tmp_path):
    """Full pipeline: Zotero fetch -> filter -> retrieve -> rerank -> TLDR -> web report."""
    ...
    # 4. Stub sleep (reranker/retriever)
    ...
    assert (docs_root / "_sidebar.md").exists()
    assert list(docs_root.glob("*/**/README.md"))
    report_jsons = list(archive_root.glob("*/recommend/*.standard.json"))
    assert len(report_jsons) == 1
    assert "E2E Paper" in report_jsons[0].read_text(encoding="utf-8")


def test_run_no_papers_generates_empty_report(config, monkeypatch, tmp_path):
    """When no papers are found, an empty web report is still generated."""
    from omegaconf import open_dict
    ...
    report_jsons = list((tmp_path / "archive").glob("*/recommend/*.standard.json"))
    assert len(report_jsons) == 1
